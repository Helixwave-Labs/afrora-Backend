...
from app.infrastructure.services.cache_service import get_cached_data, set_cached_data, delete_cache_pattern

async def invalidate_product_cache(request: Request, product_id: Optional[str] = None):
...
async def get_all_products(
    ...
    products_query = select(models.Product).options(
    ...
    result = await db.execute(products_query.offset(skip).limit(limit))
...
async def get_product(request: Request, product_id: str, db: AsyncSession = Depends(get_read_db)):
    ...
    stmt = select(models.Product).options(
        joinedload(models.Product.owner),
        joinedload(models.Product.subcategory),
        joinedload(models.Product.reviews).joinedload(models.Review.user)
...
async def update_product(
    ...
    if subcategory_id:
        sub_res = await db.execute(select(models.SubCategory).filter(models.SubCategory.id == subcategory_id))
        db_subcategory = sub_res.scalars().first()
        if not db_subcategory:
            raise HTTPException(status_code=404, detail="Subcategory not found")
